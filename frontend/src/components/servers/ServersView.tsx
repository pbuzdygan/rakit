...
function InventoryModal({ open, profile, onClose, onPublished }: { open: boolean; profile: SemaphoreProfile | null; onClose: () => void; onPublished: () => void }) {
  ...
  const keepRakit = useMutation({ mutationFn: () => Api.semaphore.keepRakitInventory(profile!.id, diff.data?.remoteHash || ''), onSuccess: onPublished });
  const keepRemote = useMutation({ mutationFn: () => Api.semaphore.keepRemoteInventory(profile!.id, diff.data?.remoteHash || ''), onSuccess: onPublished });
  const busy = diff.isLoading || keepRakit.isPending || keepRemote.isPending;
  const previewReady = Boolean(diff.data?.remoteHash);
  const mutationError = keepRakit.error || keepRemote.error;
  return <ModalBase open={open} onClose={onClose} title="Choose inventory source" eyebrow="First synchronization" subtitle="Choose which side is authoritative. The other side will be aligned with your selection." size="lg">
    <div className="ops-inventory-compare">
      <div className="ops-inventory-choice">
        <span>Current Semaphore inventory</span>
        <pre>{diff.isLoading ? 'Loading…' : diff.data?.remote || '(empty)'}</pre>
        <p>Import compatible hosts and replace local inventory groups. Matching aliases keep their additional Rakit metadata; managed hosts missing remotely become unmanaged.</p>
        <button className="ops-button ops-button--secondary" disabled={busy || !previewReady || !diff.data?.remote?.trim()} onClick={() => keepRemote.mutate()}>{keepRemote.isPending ? 'Importing…' : 'Keep Remote Inventory'}</button>
      </div>
      <div className="ops-inventory-choice">
        <span>Inventory generated by Rakit</span>
        <pre>{diff.isLoading ? 'Loading…' : diff.data?.desired || '(empty)'}</pre>
        <p>Keep the local Rakit catalogue and replace the selected Semaphore inventory with this generated version.</p>
        <button className="ops-button ops-button--danger" disabled={busy || !previewReady} onClick={() => keepRakit.mutate()}>{keepRakit.isPending ? 'Publishing…' : 'Keep Rakit Inventory'}</button>
      </div>
    </div>
    {diff.error || mutationError ? <div className="ops-error-banner">{readApiError((diff.error || mutationError) as Error)}</div> : null}
    <div className="ops-modal-actions"><button className="ops-button ops-button--secondary" disabled={keepRakit.isPending || keepRemote.isPending} onClick={onClose}>Cancel</button></div>
  </ModalBase>;
}
